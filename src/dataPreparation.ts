...
function transformData(rows: any[], decisionId: string, dmnFile: string): OutputJSON {
    ...
    const rowIndexKey = Object.keys(rows[0]).find(key => key === 'rowIndex');
    ...
    rows.forEach((row, index) => {
        ...
        const rowIndexValue = rowIndexKey ? row[rowIndexKey] : (index + 1).toString();
...
export async function processCSVFiles(): Promise<void> {
